...
export default function Home() {
    ...
    useEffect(() => {
        ...
        fetch(url)
            .then(response => {
                ...
                return response.text(); // Get file content as text
            })
            .then(csvData => {
                // Parse CSV data with PapaParse
                Papa.parse(csvData, {
    ...
    const handleSeasonChange = (season, data) => {
        ...
        filteredData.forEach(player => {
            const team = player.team_abbreviation;
            const points = parseFloat(player.pts) || 0; // Points per game
            const rebounds = parseFloat(player.reb) || 0; // Rebounds
            const assists = parseFloat(player.ast) || 0; // Assists
            const gamesPlayed = parseInt(player.gp) || 0; // Games played
        ...
        // Update state to trigger visualization
        setTeamTotalScores(teamTotalScoresData);
        const topPoints = filteredData.slice().sort((a, b) => (parseFloat(b.pts) || 0) - (parseFloat(a.pts) || 0)).slice(0, 5);
        const topRebounds = filteredData.slice().sort((a, b) => (parseFloat(b.reb) || 0) - (parseFloat(a.reb) || 0)).slice(0, 5);
        const topAssists = filteredData.slice().sort((a, b) => (parseFloat(b.ast) || 0) - (parseFloat(a.ast) || 0)).slice(0, 5);
        ...
        const maxBarHeight = height - margin.top - margin.bottom;
        const maxScore = d3.max(teamTotalScoresData, d => d.totalScore) || 100; // Default maxScore to 100 if undefined
        const barHeight = maxBarHeight / teamTotalScoresData.length;
        ...
        // Add player names for top 5 players within each team
        bars.each(function(d, i) {
            ...
            topPlayers.forEach(player => {
                ...
                // Calculate font size dynamically
                const fontSize = Math.max(8, Math.min(player.player.length * 6, 12)); // Adjust font size as needed
                const playerNameX = cumulativeX + 5;
                const playerNameY = barHeight / 2;

                // Check if there's enough space to display the name
                if (fontSize * player.player.length <= playerBarWidth) {
                    currentBar.append('text')
                        .attr('x', playerNameX)
                        .attr('y', playerNameY)
                        .attr('dy', '.35em')
                        .text(player.player)
        ...
        const yScale = d3.scaleBand()
            .domain(teamTotalScoresData.map(d => d.team))
        ...
        // Optionally, add labels or other SVG elements here
    };
    ...
    const playerList = useMemo(() => {
        ...
        // Step 2: Sort players by points, assists, and rebounds
        const sortedByPts = [...currentSeasonPlayers].sort((a, b) => (parseFloat(b.pts) || 0) - (parseFloat(a.pts) || 0));
        const sortedByAst = [...currentSeasonPlayers].sort((a, b) => (parseFloat(b.ast) || 0) - (parseFloat(a.ast) || 0));
        const sortedByReb = [...currentSeasonPlayers].sort((a, b) => (parseFloat(b.reb) || 0) - (parseFloat(a.reb) || 0));
    ...
    // Handle player click
    const handlePlayerClick = (player) => {
        const playerInfo = seasonData.find(p => p.player_name === player.player && p.season === selectedSeason);
        if (playerInfo) {
            setSelectedPlayer(playerInfo);
        }
    };
    ...
    return (
        <div className="container mt-4" style={{ position: 'relative' }}>
            <div className="row">
                {/* Left section: Title, Season selection, and Chart */}
                <div className="col-md-12">
                    <h1 className="mb-4">NBA Team Total Scores</h1>
                    <div className="mb-3">
                        <label htmlFor="seasonSelect" className="form-label">Select Season</label>
                        <select
                            id="seasonSelect"
